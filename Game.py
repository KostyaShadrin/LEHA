...
class Snaryad(AntiHero):
    ...
    def check(self, x, y):
        if (self.x - x) ** 2 + (self.y - y) ** 2 <= (self.r) ** 2 and self.live:
            return 1
        else:
            return 0
...
Decor = [raspolojenie_butylok(0, 0, 100), raspolojenie_butylok(0, 0, 100), raspolojenie_butylok(0, 0, 100),
    ...
    for steclo in Decor[i]:
        polygon(screen, COLORS[9],
                [(x + steclo[0] + 0.05 * r // 2, y + steclo[1] + 0.5 * r // 2),
                 ...
                 (x + steclo[0] + 0.15 * r // 2, y + steclo[1] - 0.5 * r // 2),
                 (x + steclo[0] - 0.15 * r // 2, y + steclo[1] - 0.5 * r // 2),
                 (x + steclo[0] - 0.15 * r // 2, y + steclo[1] + 0.2 * r // 2),
                 (x + steclo[0] - 0.05 * r // 2, y + steclo[1] + 0.3 * r // 2),
                 (x + steclo[0] - 0.05 * r // 2, y + steclo[1] + 0.5 * r // 2)])
...
class Bullets:
    ...
    def risyi(self):
        polygon(screen, COLORS[player.tsvet_planseta],
                [self.new_coord(0, 0), self.new_coord(-0.2, 0.05), self.new_coord(-1, 0.05), self.new_coord(-1, -0.05),
                 self.new_coord(-0.2, -0.05)])
        polygon(screen, BLACK,
                [self.new_coord(0, 0), self.new_coord(-0.2, 0.05), self.new_coord(-1, 0.05), self.new_coord(-1, -0.05),
                 self.new_coord(-0.2, -0.05)], 1)
        circle(screen, BLACK, self.new_coord(0, 0), 2)


def draw_scren():
    screen.fill([136, 136, 136])
    for skamya in range(10):
        lavka(100 + 300 * (skamya % 5), 50 + 620 * (skamya // 5), 70, skamya)
...
while not finished:
    ...
    for j in range(len(Magazin)):
        for i in range(len(Dolgi)):
            if Dolgi[i].check(Magazin[j].x, Magazin[j].y):
                score += 1
                Dolgi[i] = SharOdin()
        for ik in range(k_antihero):
            if Vragi[ik].check(Magazin[j].x, Magazin[j].y):
                score += 1
                Vragi[ik] = AntiHero()
